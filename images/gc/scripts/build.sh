...
# kubectl 现在在 Dockerfile 中直接下载，不再需要构建脚本下载
# 如果需要指定 kubectl 版本，可以通过 --build-arg KUBECTL_VERSION=v1.29.0 传递
...
# 获取 kubectl 版本（如果未指定）
KUBECTL_VERSION="${KUBECTL_VERSION:-$(kubectl version --client --short 2>/dev/null | grep -oP 'v\d+\.\d+\.\d+' | head -1 || echo "v1.29.0")}"
