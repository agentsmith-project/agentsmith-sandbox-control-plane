...
images_to_update = {
    ...
    'sandbox-gc': parse_image('${gc_image}')
}
...
update_kustomization_images() {
    ...
    # Try kustomize first
    if update_images_with_kustomize "$overlay_dir" "$manager_image" "$runner_image" "$gc_image"; then
        log_success "使用 kustomize 更新镜像"
        return 0
    fi
    
    # Fallback to Python
    if update_images_with_python "$overlay_dir" "$manager_image" "$runner_image" "$gc_image"; then
